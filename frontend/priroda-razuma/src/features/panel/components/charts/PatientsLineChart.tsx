...
import { ChartProps, PatientResponse } from '../interfaces/charts.types';
...
export function PatientsLineChart({ dateRange }: ChartProps) {
  ...
  const groupByMonths = (data: PatientResponse[]) => {
    const now = new Date();
    ...
    data.forEach(({ date, patient_count }) => {
      const d = new Date(date);
      const monthKey = `${d.getFullYear()}-${(d.getMonth() + 1).toString().padStart(2, '0')}`;
      monthlyData.set(monthKey, (monthlyData.get(monthKey) || 0) + patient_count);
    });
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const { data } = await api.get<PatientResponse[]>(`/statistics/patients/dynamics/${days}`);
        ...
        if (dateRange === 'year') {
          ...
          counts = data.map(({ patient_count }) => patient_count);
        }
        ...
        setChartData({
          ...
          datasets: [{
            ...
            data: counts,
            borderColor: '#D3E29F',
            backgroundColor: 'rgba(211, 226, 159, 0.2)',
            tension: 0.4,
            pointRadius: 3,
            pointHoverRadius: 5,
          }]
