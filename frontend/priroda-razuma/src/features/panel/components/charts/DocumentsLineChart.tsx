...
import { ChartProps, DocumentResponse } from '../interfaces/charts.types';
...
export function DocumentsLineChart({ dateRange }: ChartProps) {
  ...
  const groupByMonths = (data: DocumentResponse[]): Array<{ month: string, count: number }> => {
    const now = new Date();
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const { data } = await api.get<DocumentResponse[]>(`/statistics/documents/${days}`);
        ...
        setChartData({
          ...
          datasets: [{
            ...
            data: counts,
            borderColor: '#7DC459',
            backgroundColor: 'rgba(125, 196, 89, 0.2)',
            tension: 0.4,
            pointRadius: 3,
            pointHoverRadius: 5,
          }]
