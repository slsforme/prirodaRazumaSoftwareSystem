...
async def write_influx_point(measurement: str, fields: dict, tags: dict = None):
    ...
    try:
        await asyncio.to_thread(
            write_api.write, bucket=INFLUX_SETTINGS["bucket"], record=point
        )
...
NEW_PATIENTS_TOTAL = Counter("new_patients_registered_total", "New patients registered")
...
SYSTEM_MEMORY = Gauge(
    "system_memory_usage_percent", "System memory usage percentage", ["memory_type"]
)

CPU_USAGE = Gauge(
    "system_cpu_usage_percent", "System CPU usage percentage", ["cpu_core"]
)
    ...
    try:
        ...
        for idx, usage in enumerate(cpu_percent):
            ...
            await write_influx_point(
                "system_cpu",
                {"usage_percent": usage},
                {"core": str(core_number), "name": f"Ядро {core_number}"},
            )
        ...
        await write_influx_point(
            "patients", {"new_registrations": new_patients_count}, {"type": "hourly"}
        )
        ...
        if ages:
            ...
            await write_influx_point(
                "patients_age", {"average": avg_age}, {"metric": "mean"}
            )
            AVG_PATIENT_AGE.set(avg_age)

            buckets = [0, 3, 6, 9, 12, 15, 18]
            age_counts = {
                f"{buckets[i]}-{buckets[i+1]}": 0 for i in range(len(buckets) - 1)
            }
            for age in ages:
            ...
            for bucket, count in age_counts.items():
                await write_influx_point(
                    "patients_age_groups", {"count": count}, {"age_group": bucket}
                )
        ...
        for role_name, count in roles:
            await write_influx_point(
                "user_roles", {"count": count}, {"role": role_name}
            )
        ...
        await write_influx_point(
            "documents", {"total": total_docs_count}, {"type": "all"}
        )
        ...
        for doc_type, count in docs_by_type:
            await write_influx_point(
                "document_types", {"count": count}, {"type": doc_type.value}
            )
